...
async def read_dataset(query: str):
    try:
        ...
        result_data_frame = result_data_frame.head(config["num_documents_for_llm"])

        total_string = "".join(result_data_frame["Description"].tolist())

        llm_summary = get_llm_result_from_string(llm_chain, total_string)
        # llm_summary = ""
